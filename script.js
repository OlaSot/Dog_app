document.addEventListener('DOMContentLoaded', function () {
    ...
    function addMessage(sender, message) {
        ...
        messageElement.textContent = sender === 'user' ? 'Вы: ' + message : 'Владелец собаки: ' + message;
    ...
    function fetchRandomUserAndDog(breed = '') {
        fetch(API_RANDOMUSER_URL)
            ...
            .then(data => {
                ...
                const dogUrl = breed ? `${API_BASE_URL}/breed/${breed}/images/random` : `${API_BASE_URL}/breeds/image/random`;
                fetch(dogUrl)
                    .then(response => {
                        if (!response.ok) {
